...
namespace Yolo{
    ...
    const int NUM_BOX_ELEMENT = 39;//7+32      // left, top, right, bottom, confidence, class, keepflag
    ...
    static __global__ void decode_kernel(float* predict, int num_bboxes, int num_classes, float confidence_threshold, float* invert_affine_matrix, float* parray, int max_objects){  
        ...
        float* mask_pointer = pitem+5+num_classes-32;
        float confidence        = *class_confidence++;
        int label               = 0;
        for(int i = 1; i < num_classes-32; ++i, ++class_confidence){
        ...
        //float* mask_pointer = pitem + 17; //类别该了也要改
        ...
        *pout_item++ = *mask_pointer++;//mask 32
        *pout_item++ = *mask_pointer++;//mask 32
        *pout_item++ = *mask_pointer++;//mask 32
        *pout_item++ = *mask_pointer++;//mask 32
        *pout_item++ = *mask_pointer++;//mask 32
