...
#define _TOSTRING(buf, fmt, val)	\
	...
	time_t lastModify(const string& file){
	...
	bool isFolder(const string& file){
	...
	void drawbbox(cv::Mat& image, const BBox& bbox, DrawType drawType, const string& labelText){
		...
		if (drawType != DrawType::Empty) {
			...
			if (textPostition.y < 10) {
				textPostition.y = bbox.b;
			}
			...
			cv::putText(image, text, textPostition, 0, 0.45, cv::Scalar::all(255), textThickness, 16);
		}
	}
	...
	cv::Mat decodeImage(const string& data, int flags){

		cv::Mat output;
		try{
			output = cv::imdecode(cv::Mat(1, data.size(), CV_8U, (uchar*)data.data()), flags);
		}catch(...){
			INFOE("Decode image fail, data.size = %d, flags = %d", data.size(), flags);
		}
		return output;
	}
	...
	BinIO::BinIO(FILE* fhandle) {
		this->openFile(fhandle);
	}

	void BinIO::openFile(FILE* file) {
		f_ = file;
		flag_ = FileIO;

		readModeEndSEEK_ = 0;
		if (f_ != nullptr) {
			size_t cursor = ftell(f_);
			fseek(f_, 0, SEEK_END);
			readModeEndSEEK_ = ftell(f_);
			fseek(f_, cursor, SEEK_SET);
		}
	}
	...
	int BinIO::read(void* pdata, size_t length){
		...
		if (flag_ == FileIO) {
			size_t nread = fread(pdata, 1, length, f_);
			opstate_ = opstate_ && (nread == length);
			if (nread != length) {
				INFO("Fail to read, except size: %d, real is %d", length, nread);
			}
			return nread;
		}
		else if (flag_ == MemoryRead) {
	...
	int BinIO::write(const void* pdata, size_t length){
		...
		if (flag_ == FileIO) {
			size_t nwrite = fwrite(pdata, 1, length, f_);
			opstate_ = opstate_ && (nwrite == length);
			if (nwrite != length) {
				INFO("Fail to write, except size: %d, real is %d", length, nwrite);
			}
			return nwrite;
		}
		else if (flag_ == MemoryWrite) {
	...
	BinIO& BinIO::operator >> (cv::Mat& value) {
		...
		if (flag_ == FileIO) {
			...
			int dims[32] = {-1};
			if (read(dims, info[1] * sizeof(int)) != info[1] * sizeof(int)) {
				value = matrix;
				return *this;
			}

			matrix.create(info[1], dims, info[2]);
			bool ok = read(matrix.data, info[3]) == info[3];
			if (!ok) matrix.release();
			value = matrix;
		}
		else{
			opstate_ = false;
			INFOE("Unsupport flag for load matrix");
		}
		return *this;
	}
	...
	BinIO& BinIO::operator << (const cv::Mat& value){
		...
		if (flag_ == FileIO) {
			...
			if (value.isSubmatrix()) {
				w = value.clone();
			}
			else if (value.dims == 2) {
				if (value.step.p[1] * value.size[1] != value.step.p[0])
					w = value.clone();
			}
			...
			//dim, type
			int info[] = {(int)0xCCABABCC, value.dims, value.type(), total};
			write(info, sizeof(info));
			write(value.size, sizeof(int) * value.dims);
			write(w.data, total) == total;
		}
		else {
			opstate_ = false;
			INFOE("Unsupport flag for save matrix");
		}
		return *this;
	}
...
#define __GetTimeBlock						\
	...
	int getWeekDayByName(char* wday){
	...
	string nowGMTime() {
		return std::move(gmtime(time(nullptr)));
	}
	...
	time_t string2time_t(string string_time) {
		const char* cha = (const char*)string_time.data();								// 将string转换成char*。
		tm tm_;																			// 定义tm结构体。
	...
	void __log_func(const char* file, int line, int level, const char* fmt, ...) {
		...
		int n = sprintf(buffer, "[%s]", now.c_str());
